"""F-beta scorer customized for the MABe challenge submission format.
Example: from F_Beta import score
"""
import json
...
def single_lab_f1(lab_solution: pl.DataFrame, lab_submission: pl.DataFrame, beta: float = 1) -> float:
    ...
    for video in lab_solution['video_id'].unique():
        ...
        for row in lab_submission.filter(pl.col('video_id') == video).to_dicts():
            # Labels are sparse; skip prediction keys not in the active labels.
            ...
            if predicted_mouse_pairs[prediction_pair].intersection(new_frames):
                # An agent may have multiple targets per frame but only one action per target per frame.
...
def mouse_fbeta(solution: pd.DataFrame, submission: pd.DataFrame, beta: float = 1) -> float:
    ...
    # Align on video IDs since row IDs are unreliable across public/private splits.
