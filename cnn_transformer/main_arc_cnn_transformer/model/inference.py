"""Generate Kaggle submission.csv for the MABe behavior-detection model.

Usage: python inference.py --model_path <ckpt.pt> [--output submission.csv]
"""
...
def smooth_probs(probs: np.ndarray, window: int) -> np.ndarray:
    """Centered moving average along the time axis; window <= 1 disables."""
...
def enforce_pair_no_overlap(rows: List[Dict]) -> List[Dict]:
    ...
    Conflicts are resolved by per-interval mean-probability `confidence`; the
    lower-confidence side gets trimmed (and dropped if length falls below 1).
    """
    if not rows:
        return rows
        ...
        for r in group:
            ...
            for a in accepted:
                ...
                if r["confidence"] >= a["confidence"]:
                    ...
                    if a_s < s and a_e > e:
                        # Accepted fully contains new → split accepted.
                        right_half = dict(a)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--smooth_window", type=int, default=5,
                    help="Centered MA window; 1 disables. Even values bumped odd.")
    ...
    ap.add_argument("--thresholds_json", default=None,
                    help="best_thresholds.json from training; looks next to --model_path if omitted.")
...
def main() -> None:
    ...
    # Inference window must match the trained window for correct rel_pos_bias coverage.
    if hasattr(model, "encoder_layers") and len(model.encoder_layers) > 0:
        ws_model = int(
            model.encoder_layers[0].attention.rel_pos_bias.num_embeddings + 1
        ) // 2
    ...
    # Vocabs come from train.csv ONLY (must match training time); test-only
    # values map to UNK_IDX automatically.
    pair_lookup: Dict | None = None
    if model.meta_embeddings:
    ...
    for row in tqdm(test_rows, desc="Test videos"):
        ...
        for (agent_id, target_id), allowed_actions in pair_whitelist.items():
            ...
            # Per-frame argmax over allowed actions: each frame "owned" by at
            # most one action; enforce_pair_no_overlap catches close_mask leakage.
