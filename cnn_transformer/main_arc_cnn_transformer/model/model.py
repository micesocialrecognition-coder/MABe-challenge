"""CNN+Transformer per-frame multi-label behavior model (sigmoid head).

Usage: python model.py <data_dir> <model_dir> [--num_epochs ...]
"""
...
# Body part order resolved lazily to avoid an import cycle and to stay
# correct if preprocess_features.ALL_PARTS changes.
...
def _resolve_lr_pairs() -> List[Tuple[int, int]]:
    """Indices (within 0..N_PARTS-1) of left/right body-part pairs to swap on flip."""
...
class BehaviorDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Falls back to normal slicing when not enough source frames remain.
        ...
        # Geometric augmentations operate on the 176-d raw layout only; in concat
        # mode the embedding half is left as-is (Path 2 — see deferred memo).
...
def _overall_fbeta_from_pair_data(
    ...
    """Competition F-beta: per-action Fb pooled in-lab, mean over present actions, mean over labs."""
...
def _tune_per_lab_action(
    ...
    for lab_id, pair_indices in by_lab.items():
        for action in range(num_classes):
            ...
            # No positives → nothing to tune; defaults to 0.5 via .get() in caller.
            if not labels.any():
                continue

            # Anchor at t=0.5. Only swap if strictly better → guarantees per-(lab,action) Fb >= Fb@0.5.
            preds_05 = probs > 0.5
...
def validate(
    ...
    """Aggregate per-pair logits across overlapping windows, sweep per-(lab,action) threshold.

    Returns: (avg_loss, fb_at_0.5, fb_tuned, thresholds_dict).
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("model_dir", nargs="?", default="checkpoints/arc_cnn_transformer")
