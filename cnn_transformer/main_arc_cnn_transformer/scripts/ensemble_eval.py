"""Average per-frame probabilities across checkpoints, then re-tune thresholds.

Usage:
    python3 cnn_transformer/main_arc_cnn_transformer/scripts/ensemble_eval.py \
        --runs <run_dir1> <run_dir2> ... \
        --data_dir <path/to/processed> --train_csv <path/to/train.csv> \
        --output_path <out>/thresholds.json
"""
...
def read_features_config(config_path: str) -> tuple[str, str | None]:
    with open(config_path) as f:
        c = json.load(f)
    return c.get("features_mode", "raw"), c.get("embeddings_dir")
...
def compute_pair_probs(
    ...
    embed_dim = read_embedding_dim(embeddings_dir) if features_mode != "raw" else 0
    ...
    in_dim = feature_input_dim(features_mode, embed_dim)
...
def main():
    ...
    for run_dir in args.runs:
        ...
        f_mode, emb_dir = read_features_config(cfg)
        print(f"\n=== {run_dir}  window={w} stride={s}  features_mode={f_mode}"
              f"{('  embeddings_dir=' + emb_dir) if emb_dir else ''} ===")
        pair_keys, avg_probs, labels_buf, loss_masks, num_classes, action_list = \
            compute_pair_probs(
                ckpt, val_videos, args.data_dir, pair_lookup,
                w, s, args.batch_size, args.num_workers, device,
                meta_vocab_sizes, meta_slot_to_table,
                features_mode=f_mode, embeddings_dir=emb_dir,
            )
